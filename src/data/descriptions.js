export const categoryDescriptions = {
  E1: {
    title: 'E1: Klimaforandringer',
    description: 'Detaljeret beskrivelse af klimaforandringer...'
  },
  E2: {
    title: 'E2: Forurening',
    description: 'Detaljeret beskrivelse af forurening...'
  },
  E3: {
    title: 'E3: Vand- og havressourcer',
    description: 'Detaljeret beskrivelse af vand- og havressourcer...'
  },
  E4: {
    title: 'E4: Biodiversitet',
    description: 'Detaljeret beskrivelse af biodiversitet...'
  },
  E5: {
    title: 'E5: Cirkulær økonomi',
    description: 'Detaljeret beskrivelse af cirkulær økonomi...'
  },
  S1: {
    title: 'S1: Egen arbejdsstyrke',
    description: 'Detaljeret beskrivelse af egen arbejdsstyrke...'
  },
  S2: {
    title: 'S2: Arbejdere i værdikæden',
    description: 'Detaljeret beskrivelse af arbejdere i værdikæden...'
  },
  S3: {
    title: 'S3: Påvirkede samfund',
    description: 'Detaljeret beskrivelse af påvirkede samfund...'
  },
  S4: {
    title: 'S4: Forbrugere',
    description: 'Detaljeret beskrivelse af forbrugere...'
  },
  G1: {
    title: 'G1: Forretningsetik',
    description: 'Detaljeret beskrivelse af forretningsetik...'
  },
};

export const questionDescriptions = {
  // DVA Questions (Impact)
  1: { description: 'Gælder hvis virksomheden fx bruger firmabiler, har produktionsudstyr, opvarmer lokaler med olie/gas, eller flyver på firmarejse. Kontorarbejde uden transport tæller ikke.', typicalIndustry: 'Transport, byggeri, produktion, landbrug, detail med transport' },
  2: { description: 'Spørgsmål 2 beskrivelse...', typicalIndustry: 'Branche for spørgsmål 2...' },
  3: { description: 'Spørgsmål 3 beskrivelse...', typicalIndustry: 'Branche for spørgsmål 3...' },
  // ... add all DVA question descriptions here

  // DVA Questions (Finansiel)
  101: { description: 'Spørgsmål 101 beskrivelse...', typicalIndustry: 'Branche for spørgsmål 101...' },
  102: { description: 'Spørgsmål 102 beskrivelse...', typicalIndustry: 'Branche for spørgsmål 102...' },
  // ... add all DVA question descriptions here
};
