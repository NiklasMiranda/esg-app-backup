...
function DashboardHeader({
  isLoggedIn,
  onLogout,
  newTotalCompletionPercentage,
  onToggleNav,
  userCompanyName,
  isNavOpen,
}) {
  // Added onToggleNav, userCompanyName, isNavOpen
  ...
  return (
    <div className="esg-bg-[#0b3954] esg-p-2 esg-flex-shrink-0 esg-border-b-2 esg-border-white">
      <div className="esg-flex esg-justify-between esg-items-center esg-px-4 esg-py-2">
        {' '}
        {/* Adjusted padding */}
        ...
        <div className="esg-flex esg-items-center">
          <img
            src={ESGLogo}
            alt="ESG Score Logo"
            className="esg-h-14 esg-mr-4"
          />{' '}
          {/* Logo */}
          <button
            onClick={onToggleNav}
            className="esg-text-white focus:esg-outline-none esg-transition-transform esg-duration-300 esg-ease-in-out"
          >
            {isNavOpen ? (
              <HiX className="esg-h-6 esg-w-6" />
            ) : (
              <HiMenu className="esg-h-6 esg-w-6" />
            )}{' '}
        ...
        <div className="esg-relative">
          ...
          >
            <span className="esg-mr-2">
              {userCompanyName || 'Din Virksomhed'}
            </span>{' '}
            {/* Company Name */}
            <svg
