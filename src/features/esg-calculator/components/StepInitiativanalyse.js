import { fetchDocuments, mapDocumentsToAnswer, uploadDocument } from 'api';
...
import CustomPolarChart from 'shared/components/charts/CustomPolarChart';
import NivoLikeMarimekkoChart from 'shared/components/charts/NivoLikeMarimekkoChart';
...
function DocumentationSidebar({
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      case 'approved':
        return (
          <FaCheckCircle className="esg-text-green-500" title="Godkendt" />
        );
      case 'incomplete':
        return (
          <FaExclamationTriangle
            className="esg-text-red-500"
            title="Mangelfuldt"
          />
        );
      default:
        return (
          <FaHourglassHalf className="esg-text-gray-400" title="Ikke-tjekket" />
        );
    }
  };
  ...
  return (
    <div className="esg-bg-white esg-p-6 esg-rounded-lg esg-shadow-md esg-sticky esg-top-8">
      <h2 className="esg-text-lg esg-font-bold esg-mb-4 esg-border-b esg-pb-2">
        Dokumentation
      </h2>
      ...
      <div className="esg-mb-6">
        <div className="esg-flex esg-justify-between esg-items-center esg-mb-1">
          <span className="esg-text-md esg-font-medium esg-text-gray-700">
            Dokumentationsgrad
          </span>
          <span className="esg-text-md esg-font-medium esg-text-blue-700">
            {Math.round(progressPercentage)}%
          </span>
        </div>
        <div className="esg-w-full esg-bg-gray-200 esg-rounded-full esg-h-2.5">
          <div
            className="esg-bg-blue-600 esg-h-2.5 esg-rounded-full esg-transition-all esg-duration-500"
      ...
      <div className="esg-mb-6">
        <label
          className={`esg-flex esg-flex-col esg-items-center esg-px-4 esg-py-4 esg-bg-gray-50 esg-text-blue-600 esg-rounded-lg esg-border-2 esg-border-dashed esg-border-blue-300 esg-cursor-pointer hover:esg-bg-blue-50 esg-transition-colors ${isUploading ? 'esg-opacity-50 esg-cursor-not-allowed' : ''}`}
        >
          <FaFileUpload className="esg-w-6 esg-h-6 esg-mb-1" />
          <span className="esg-text-md esg-font-bold">
            {isUploading ? 'Uploader...' : 'Upload dokument'}
          </span>
          <input
            type="file"
            className="esg-hidden"
      ...
      <div className="esg-mb-6">
        <h3 className="esg-text-md esg-font-bold esg-text-gray-500 esg-uppercase esg-mb-3">
          Uploadet dokumentation
        </h3>
        <ul className="esg-space-y-2">
          {pendingDocs.length === 0 ? (
            <li className="esg-text-md esg-text-gray-400 esg-italic">
              Ingen nye dokumenter
            </li>
          ) : (
            pendingDocs.map((doc) => (
              <li
                key={doc.id}
                className="esg-flex esg-items-center esg-justify-between esg-p-2 esg-bg-gray-50 esg-rounded esg-text-md"
              >
                <span
                  className="esg-truncate esg-max-w-[150px]"
                  title={cleanFilename(doc.file)}
                >
      ...
      <div>
        <h3 className="esg-text-md esg-font-bold esg-text-gray-500 esg-uppercase esg-mb-3">
          Tjekket dokumentation
        </h3>
        <ul className="esg-space-y-2">
          {checkedDocs.length === 0 ? (
            <li className="esg-text-md esg-text-gray-400 esg-italic">
              Ingen behandlede dokumenter
            </li>
          ) : (
            checkedDocs.map((doc) => (
              <li
                key={doc.id}
                className="esg-flex esg-flex-col esg-p-2 esg-bg-gray-50 esg-rounded esg-text-md"
              >
                ...
                {doc.admin_comment && (
                  <p className="esg-mt-1 esg-text-sm esg-text-red-600 esg-italic">
                    "{doc.admin_comment}"
                  </p>
                )}
              </li>
...
function StepInitiativanalyse({
  ...
  const refreshDocuments = useCallback(async () => {
    try {
      ...
      const filteredDocs = docs.filter(
        (d) => groupTopics.includes(d.topic) || d.topic === activeIaGroup,
      );
  ...
  const progressPercentage = useMemo(() => {
    const questionsInThisGroup = iaQuestions.filter(
      (q) => q.sub_category.label === activeIaGroup,
    );
    const answeredInThisGroup = questionsInThisGroup.filter(
      (q) => iaAnswers[q.id]?.is_answered,
    );

    if (answeredInThisGroup.length === 0) return 0;

    const documentedCount = answeredInThisGroup.filter(
      (q) => iaAnswers[q.id]?.documents && iaAnswers[q.id].documents.length > 0,
    ).length;
  ...
  const handleDocMapping = async (questionId, docId) => {
    ...
    try {
      ...
      if (!answerId) {
        throw new Error(
          'Svaret gemmes stadig i databasen. Prøv igen om et øjeblik.',
        );
      }
    ...
    <div className="esg-grid esg-grid-cols-1 lg:esg-grid-cols-12 esg-gap-6">
      ...
      <div className="lg:esg-col-span-6">
        <div className="esg-bg-white esg-p-6 esg-rounded-lg esg-shadow-md">
          ...
          {Object.entries(groupedQuestionsByTopic).map(([topic, questions]) => (
            <div
              key={topic}
              className="esg-mb-6 esg-border esg-border-gray-100 esg-rounded-lg"
            >
              <button
              ...
              >
                <span>{topic}</span>
                <svg
                  className={`esg-w-4 esg-h-4 esg-transition-transform ${openSections[topic] ? 'esg-rotate-180' : ''}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                ...
                <div className="esg-p-4 esg-grid esg-grid-cols-1 md:esg-grid-cols-2 esg-gap-4">
                  {questions.map((q) => {
                    ...
                    return (
                      <div
                        key={q.id}
                        className="esg-bg-white esg-p-4 esg-rounded-lg esg-border esg-border-gray-50 esg-shadow-sm hover:esg-shadow-md esg-transition-shadow"
                      >
                        <div className="esg-flex esg-justify-between esg-items-start esg-mb-3">
                          <div className="esg-flex-1">
                            <p className="esg-text-sm esg-text-gray-400 esg-font-bold">
                              {q.number}
                            </p>
                            <p className="esg-text-md esg-leading-snug">
                              {q.text}
                            </p>
                          </div>
                          <input
                            ...
                            checked={isAnswered}
                            onChange={(e) =>
                              onIaAnswerChange(q.id, e.target.checked)
                            }
                        ...
                        {isAnswered && (
                          <div className="esg-mt-4 esg-pt-3 esg-border-t esg-border-gray-50">
                            <div className="esg-flex esg-items-center esg-gap-1 esg-mb-2 esg-text-gray-500">
                              ...
                              <span className="esg-text-sm esg-font-bold esg-uppercase">
                                Knyt dokumentation
                              </span>
                            </div>
                            <select
                              ...
                              onChange={(e) =>
                                handleDocMapping(q.id, parseInt(e.target.value))
                              }
                              value=""
                              disabled={isMapping}
                            >
                              <option value="">
                                Vælg fra uploadet dokumentation...
                              </option>
                            ...
                            {linkedDocs.length > 0 && (
                              <div className="esg-flex esg-wrap esg-gap-1 esg-mt-2">
                                {linkedDocs.map((ld) => (
                                  <span
                                    key={ld.id}
                                    className="esg-inline-flex esg-items-center esg-px-2 esg-py-1 esg-rounded-full esg-bg-blue-50 esg-text-blue-700 esg-text-sm"
                                  >
          ...
          <div className="esg-flex esg-justify-between esg-mt-8">
            ...
            {isLast ? (
              <button onClick={onShowResults} className="btn-primary">
                Vis Resultater
              </button>
            ) : (
      ...
      <div className="lg:esg-col-span-4">
        <div className="esg-sticky esg-top-8 esg-space-y-6">
          ...
          <div className="esg-bg-white esg-p-4 esg-rounded-lg esg-shadow-md">
            <NivoLikeMarimekkoChart
              data={
                marimekkoData?.filter(
                  (item) =>
                    iaQuestions.find((q) => q.topic === item.subcategory)
                      ?.sub_category.label === activeIaGroup,
                ) || []
              }
            />
          </div>
      ...
      <Drawer
        ...
        title={`${activeIaGroup}: ${groupTitles[activeIaGroup]}`}
      >
        <p>
          {categoryDescriptions[activeIaGroup]?.description ||
