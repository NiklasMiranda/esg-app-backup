...
import { LuFileText, LuWeight } from 'react-icons/lu';
import CircularProgress from 'shared/components/charts/CircularProgress';
import YearSelector from 'shared/components/forms/YearSelector';
...
function ESGCalculatorNav({
  ...
  const del2NavSteps = [
    ...
    ...questionGroups.map((groupKey) => ({
      key: groupKey,
      title: `${groupKey}: ${groupTitles[groupKey] || ''}`,
    })),
  ...
  const baseButtonFlex =
    'esg-flex-grow esg-basis-0 esg-py-2 esg-px-3 esg-text-sm esg-flex esg-items-center esg-justify-center esg-gap-1';
  ...
  return (
    <nav className="esg-bg-[#f4f4f4] esg-text-gray-800 esg-shadow-md">
      ...
      <div className="esg-flex esg-flex-col">
        {/* DVA and IA Buttons */}
        <div
          className={`esg-grid esg-grid-cols-2 esg-border-b-[1px] ${inactiveBorderColor}`}
        >
          <button
        ...
        >
          {/* DVA Question Groups */}
          <div
            className={`esg-flex esg-flex-row esg-items-center esg-overflow-x-auto ${activeSection === 'del1' ? activeBorderColor : inactiveBorderColor}`}
          >
            <div className="esg-flex esg-justify-start esg-flex-grow">
              {del1Steps
                .filter((step) => questionGroups.includes(step.key))
                .map((step, index, arr) => {
                  const isActive =
                    activeGroup === step.key && activeSection === 'del1';
                  console.log(
                    `DVA Subcategory: ${step.key}, isActive: ${isActive}`,
                  );
                  ...
                  return (
                    ...
                    >
                      <CircularProgress
                        percentage={categoryCompletionStatus[step.key] || 0}
                        size={24}
                      />
                      {step.key}
                    </button>
          ...
          <div className="esg-flex esg-flex-row esg-items-center esg-overflow-x-auto">
            <div className="esg-flex esg-justify-start esg-flex-grow">
              {del2NavSteps
                ...
                .map((step, index, arr) => {
                  const isActive =
                    activeGroup === step.key && activeSection === 'del2';
                  console.log(
                    `IA Subcategory: ${step.key}, isActive: ${isActive}`,
                  );
                  const hasRight = index < arr.length - 1;
                  return (
                    <button
