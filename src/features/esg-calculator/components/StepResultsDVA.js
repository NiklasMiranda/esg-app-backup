...
function StepResultsDVA({
  answers,
  criteriaWeights,
  impactFinansielCounts,
  onNext,
  onPrev,
  criterionColors,
  dvaQuestions,
  ...
  const { plottedCoordinates } = useMemo(() => {
    const results = {};
    [...new Set(dvaQuestions.map((q) => q.sub_category.label))].forEach(
      (label) => {
    ...
    console.log(
      'Results object after initialization:',
    ...
    dvaQuestions.forEach((q) => {
      if (selectedQuestions.has(q.id)) {
        ...
        else if (q.purpose === 'finansiel')
          results[q.sub_category.label].finansiel++;
      }
    });

    console.log(
      'Results object after processing answers:',
      JSON.stringify(results, null, 2),
    );
    ...
    console.log(
      'Plotted Coordinates after jitter:',
      JSON.stringify(jitteredCoordinates, null, 2),
    );
      ...
      <div className="font-raleway">
        <div className="esg-flex esg-gap-8 esg-mb-8">
          ...
          <div className="esg-flex-1 esg-bg-white esg-p-8 esg-rounded-lg esg-shadow-md">
            <h2 className="esg-text-2xl esg-font-bold esg-mb-4">
              Væsentlighedsanalyse
            </h2>
            <p className="esg-text-gray-700">
              Her kan du se en visuel repræsentation af din
              væsentlighedsanalyse, baseret på dine besvarelser. Akserne
              repræsenterer henholdsvis virkningsvæsentlighed og finansiel
              væsentlighed.
            </p>
          </div>
            ...
            <div className="esg-relative esg-w-full esg-max-w-xl esg-h-auto esg-aspect-square esg-mx-auto">
              ...
              <div className="esg-relative esg-w-full esg-h-full esg-border esg-border-gray-400 esg-grid esg-grid-cols-6 esg-grid-rows-6 esg-gap-px esg-bg-gray-200 esg-rounded-lg">
                {Array.from({ length: 36 }).map((_, i) => (
                  <div
                    key={i}
                    className="esg-bg-white esg-border esg-border-gray-200"
                  ></div>
                ))}
                ...
                {plottedCoordinates.map((item) => (
                  <div
                    ...
                    style={{
                      backgroundColor:
                        criterionColors[item.originalLabels[0]] || '#ccc',
        ...
        <div className="esg-flex esg-gap-8 esg-mb-8">
          <div className="esg-flex-1 esg-bg-white esg-p-8 esg-rounded-lg esg-shadow-md">
            ...
            <p className="esg-text-gray-700">
              Her kan du få et overblik over de forskellige kriterier, der
              afspejler deres relative betydning for din virksomhed.
            </p>
          </div>
          ...
          <div className="esg-flex-[2] esg-bg-white esg-p-14 esg-rounded-lg esg-shadow-md esg-overflow-x-auto">
            <table className="esg-min-w-full esg-bg-white esg-border esg-border-[#f4f4f4] esg-rounded-lg esg-overflow-hidden">
              ...
              <tbody>
                {Object.entries(criteriaWeights || {}).map(
                  ([label, weight]) => (
                    <tr key={label}>
